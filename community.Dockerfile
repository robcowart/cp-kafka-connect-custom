...
LABEL org.opencontainers.image.created="$BUILD_DATE" \
      ...
      org.opencontainers.image.version="5.3.1_0.0.4_community" \
...
RUN confluent-hub install --no-prompt confluentinc/connect-transforms:1.2.1 && \
    ...
    confluent-hub install --no-prompt jcustenborder/kafka-connect-redis:0.0.2.10 && \
    confluent-hub install --no-prompt jcustenborder/kafka-connect-simulator:0.1.120 && \
    confluent-hub install --no-prompt jcustenborder/kafka-connect-solr:0.1.33 && \
