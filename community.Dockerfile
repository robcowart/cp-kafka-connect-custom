...
LABEL org.opencontainers.image.created="$BUILD_DATE" \
      ...
      org.opencontainers.image.version="5.5.0_1.0.2_community" \
...
RUN confluent-hub install --no-prompt bkatwal/bkatwal-kafka-connect-solr-sink:2.0 && \
    ...
    confluent-hub install --no-prompt confluentinc/kafka-connect-hdfs:5.5.0 && \
    ...
    confluent-hub install --no-prompt f0xdx/kafka-connect-wrap-smt:0.1.0 && \
    ...
    confluent-hub install --no-prompt jcustenborder/kafka-connect-flume-avro:0.2.8 && \
    ...
    confluent-hub install --no-prompt mdrogalis/voluble:0.3.0 && \
    confluent-hub install --no-prompt microsoft/kafka-connect-iothub:0.6 && \
    confluent-hub install --no-prompt mmolimar/kafka-connect-fs:1.0.0 && \
    confluent-hub install --no-prompt mongodb/kafka-connect-mongodb:1.1.0 && \
    ...
    confluent-hub install --no-prompt riferrei/kafka-connect-pulsar:0.6.0 && \
    ...
    confluent-hub install --no-prompt snowflakeinc/snowflake-kafka-connector:1.2.3 && \
