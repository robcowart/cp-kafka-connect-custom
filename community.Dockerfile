...
LABEL org.opencontainers.image.created="$BUILD_DATE" \
      ...
      org.opencontainers.image.version="5.4.0_1.0.0_community" \
...
RUN confluent-hub install --no-prompt confluentinc/connect-transforms:1.2.1 && \
    confluent-hub install --no-prompt confluentinc/kafka-connect-avro-converter:5.4.0 && \
    ...
    confluent-hub install --no-prompt confluentinc/kafka-connect-hdfs:5.4.0 && \
    confluent-hub install --no-prompt confluentinc/kafka-connect-jdbc:5.4.0 && \
    confluent-hub install --no-prompt confluentinc/kafka-connect-s3:5.4.0 && \
    confluent-hub install --no-prompt confluentinc/kafka-connect-vertica:1.0.1-preview && \
    ...
    confluent-hub install --no-prompt jcustenborder/kafka-connect-transform-xml:0.1.0.16 && \
